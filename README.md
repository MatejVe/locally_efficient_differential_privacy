...
$$
\begin{equation*}
\max_{Q \in D_{\epsilon, k}} I_\theta(Q) = \max_{Q \in D_{\epsilon, k}} \sum_{z \in Z} \frac{\left( \sum_{x \in X} Q(z|x) p'_\theta(x) \right)^2}{\sum_{x \in X} Q(z|x) p_\theta(x)} = \max_{Q \in D_{\epsilon, k}} \sum_{z \in Z} \frac{(Q_z \cdot p'_\theta)^2}{Q_z \cdot p_\theta}.
\end{equation*}
$$
...
1. The objective function $I_\theta(Q)$ is convex in $Q$ (Steinberger (2024), Lemma 4.5).
...
$$ h(a) := a^2 \frac{y}{x} + b^2 {x}{y} - 2ab $$

and observe:

$$ h'(a) = 2a\frac{y}{x} - 2b, \quad h''(a) = 2 \frac{y}{x} > 0, $$

meaning $h$ is strictly convex. Therefore, $h(a) \geq 0$ for all $a$, and $g_\theta(Q_z)$ is convex.
